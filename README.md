...
```bash
# CPU
docker pull ghcr.io/lightonai/next-plaid:cpu-1.3.0
docker run -p 8080:8080 -v ~/.local/share/next-plaid:/data/indices \
  ghcr.io/lightonai/next-plaid:cpu-1.3.0 \
...
```bash
# GPU
docker pull ghcr.io/lightonai/next-plaid:cuda-1.3.0
docker run --gpus all -p 8080:8080 -v ~/.local/share/next-plaid:/data/indices \
  ghcr.io/lightonai/next-plaid:cuda-1.3.0 \
  --host 0.0.0.0 --port 8080 --index-dir /data/indices \
